...
def generate_pdf_report(frame: pd.DataFrame, dataset_id: str, options: ReportRequest, versions: dict[str, Any]) -> bytes:
    ...
    if options.include_profile:
        ...
        for item in [column for column in profile["columns"] if column["category"] == "numeric"][:20]: rows.append([_display(item["name"]), *[_display(item.get(key)) for key in ("sum", "mean", "median", "minimum", "maximum")]])
        story.extend([Paragraph("Numeric profile", styles["Section"]), Table(rows, repeatRows=1, style=[("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#17233f")), ("TEXTCOLOR", (0, 0), (-1, 0), colors.white), ("GRID", (0, 0), (-1, -1), .3, colors.HexColor("#ccd2df")), ("FONTSIZE", (0, 0), (-1, -1), 7), ("PADDING", (0, 0), (-1, -1), 4)])])
    if options.include_insights:
        story.append(Paragraph("Key insights", styles["Section"]))
        for item in generate_insights(frame, dataset_id)[:15]: story.extend([Paragraph(f"<b>{escape(item['title'])}</b>", styles["BodyText"]), Paragraph(escape(item["description"]), styles["BodyText"]), Spacer(1, 5)])
    if options.include_quality:
        issues = analyze_quality(frame)[:20]
        story.append(Paragraph("Data quality", styles["Section"]))
        if issues:
            rows = [["Issue", "Column", "Count", "Examples"]] + [[_display(item["issue_type"].replace("_", " ").title()), _display(item.get("column") or "Dataset"), item["count"], _display(", ".join(item["examples"][:3]))] for item in issues]
            story.append(Table(rows, repeatRows=1, colWidths=[40 * mm, 35 * mm, 18 * mm, 70 * mm], style=[("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#17233f")), ("TEXTCOLOR", (0, 0), (-1, 0), colors.white), ("GRID", (0, 0), (-1, -1), .3, colors.HexColor("#ccd2df")), ("FONTSIZE", (0, 0), (-1, -1), 7), ("VALIGN", (0, 0), (-1, -1), "TOP")]))
        else:
            story.append(Paragraph("No data-quality issues were detected by the configured checks.", styles["BodyText"]))
    if options.include_charts: story.extend([Paragraph("Chart summary", styles["Section"]), Paragraph(f"{len(profile['numeric_columns'])} numeric and {len(profile['categorical_columns'])} categorical columns are available for deterministic charts. Interactive chart data remains available in DataPilot AI.", styles["BodyText"])])
