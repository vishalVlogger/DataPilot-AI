...
Run `alembic upgrade head` before each application release. Revision `0002` creates SaaS tables and assigns pre-existing records to `LEGACY_WORKSPACE_ID`, preserving local data while preventing new users from claiming it. Revisions `0003` through `0008` add beta identity, collaboration, feedback, operations, analytics, and notification metadata. Revision `0009` adds provider-neutral subscriptions, one-time workspace trials, upgrade requests, and idempotent usage-meter keys. It does not delete or archive customer datasets when a workspace moves to a lower plan.
