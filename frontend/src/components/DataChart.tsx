...
export function DataChart({ chart, onPointClick }: { chart: ChartResponse; onPointClick?: (row: Record<string, string | number | null>) => void }) {
  ...
  return <ResponsiveContainer width="100%" height={330}><BarChart {...common}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey={chart.x_axis}/><YAxis/><Tooltip/><Bar dataKey={chart.y_axis} fill={colors[0]} radius={[6, 6, 0, 0]} onClick={(entry) => onPointClick?.(entry.payload as Record<string, string | number | null>)}/></BarChart></ResponsiveContainer>;
}
