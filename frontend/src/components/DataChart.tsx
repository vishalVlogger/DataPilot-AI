...
export function DataChart({ chart }: { chart: ChartResponse }) {
  ...
  if (chart.type === "line") return <ResponsiveContainer width="100%" height={330}><LineChart {...common}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey={chart.x_axis}/><YAxis/><Tooltip/><Legend/><Line dataKey={chart.y_axis} stroke={colors[0]} strokeWidth={3} dot={{ r: 3 }}/></LineChart></ResponsiveContainer>;
  if (chart.type === "pie") return <ResponsiveContainer width="100%" height={330}><PieChart><Tooltip/><Legend/><Pie data={chart.data} dataKey={chart.y_axis} nameKey={chart.x_axis} outerRadius={115} label>{chart.data.map((_, index) => <Cell key={index} fill={colors[index % colors.length]}/>)}</Pie></PieChart></ResponsiveContainer>;
